...
def resolve_tools() -> tuple[Path, Path]:
    ...
    candidates = [
        ...
        "/var/lib/flatpak/app/org.eclipse.sumo/x86_64/"
        + "stable/active/files/share/sumo/tools",
    ]
    tools_dir = None
    for c in candidates:
    ...
    if tools_dir is None:
        raise FileNotFoundError(
            "randomTrips.py not found; set SUMO_TOOLS to the sumo tools dir"
        )
    ...
    raise FileNotFoundError(
        "duarouter not found; add sim/vendor/bin/duarouter or set PATH"
    )
...
def corridor_edge_set(net) -> set:
    ...
    for a, b in zip(nodes, nodes[1:]):
        match = [
            ...
            and e.getFromNode().getID() == a
            and e.getToNode().getID() == b
        ]
        if not match:
...
def build_ev_route(cache: bool = True) -> tuple[list[str], list[str]]:
    ...
    if edges[0] != chosen["origin_edge"] or edges[-1] != chosen["destination_edge"]:
        raise ValueError(
            "resolved EV route endpoints do not match chosen origin/destination"
        )
...
def gen_background(
    ...
    if not quiet:
        Console().print(
            f"[green]randomTrips[/green] density={density} seed={seed} period={period}s "
...
def write_ev_rou(edges: list[str], depart: float) -> Path:
    ...
    content = (
        ...
        'decel="4.5" length="5" sigma="0.2" speedFactor="1.0" color="255,0,0"/>\n'
...
def write_cfg(
    density: str, seed: int, bg_rou: Path, ev_rou: Path, begin: float, end: float
) -> Path:
...
def summary(
    ...
    Console().print(table)
    Console().print(
        f"[green]scenario[/green] wrote {bg_rou.name} + ev.rou.xml + "
...
def build(
    ...
    if not quiet:
        ...
        Console().print(
            f"[green]scenario[/green] run with: "
...
def build_many(
    ...
    seeds: list[int],
    force: bool = False,
    begin: float = DEFAULT_BEGIN,
    end: float = DEFAULT_END,
    ev_depart: float = DEFAULT_EV_DEPART,
) -> list[tuple[str, int]]:
    ...
    for density in densities:
        for seed in seeds:
            ...
            if (
                not force
                and cfg.is_file()
                and cfg.stat().st_size > 0
                and rou.is_file()
                and rou.stat().st_size > 0
            ):
                continue
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description="amroute baseline scenario generator (R=0)"
    )
    ap.add_argument(
        "--density",
        choices=["low", "med", "high"],
        help=f"congestion level ({DENSITY_HELP})",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="override background emission period (s)",
    )
    ap.add_argument(
        "--begin",
        type=float,
        default=DEFAULT_BEGIN,
        help=f"sim begin (default {DEFAULT_BEGIN:g})",
    )
    ap.add_argument(
        "--end",
        type=float,
        default=DEFAULT_END,
        help=f"sim end (default {DEFAULT_END:g})",
    )
    ap.add_argument(
        "--ev-depart",
        type=float,
        default=DEFAULT_EV_DEPART,
